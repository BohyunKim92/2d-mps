...
pivot_vertices = [(0, 0), (3*np.pi, 0), (3*np.pi, 4*np.pi), (0, 4*np.pi)]
...
def create_solver(m_B_per_edge, m_I, N):
    ...
    alpha_k = np.array([alpha] * 4).reshape(-1, 1, 1) * (np.arange(N) + 1)
...
def plot_sigma():
    ...
    # ax.set_ylim((0, 1))
    plt.savefig('rectangle_sigma_multiple_corners.png',
...
def plot_eigenfunctions():
    ...
    plt.savefig('rectangle_eigenfunctions_multiple_corners.png',
                dpi=300, bbox_inches='tight')


def plot_cond():
    def cond_A(N, lam):
        solver = create_solver(2*N, 2*N, N)
    ...
    ax.set_yscale('log')
    ax.legend()
    plt.savefig('rectangle_cond_multiple_corners.png',
                dpi=300, bbox_inches='tight')
